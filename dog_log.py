...
# * 현재 버전   : v11.9.1 (유령 쿠키 퇴치 & 6개월 프리패스 완벽 패치)
# *********************************************************************
...
import time  # ★ 스마트폰 통신 대기 시간을 위한 필수 부품

# ==========================================
# 0. 엔진 세팅 및 파이어베이스 클라우드 연결
# ==========================================
APP_VERSION = "v11.9.1 (쿠키 패치 완료)"
...
if 'force_logout' not in st.session_state: st.session_state.force_logout = False # 유령 쿠키 방어막

# ==========================================
# 🚪 1. 중앙 현관문 (6개월 자동 로그인 완벽 통제)
# ==========================================
saved_user = cookie_manager.get(cookie="saved_username")

# ★ 유령 쿠키 방어 로직: 로그아웃 직후라면 쿠키가 남아있어도 강제로 무시합니다.
        ...
        if st.button("접속하기 🚀", use_container_width=True, type="primary"):
            if login_id and login_pw:
                ...
                if res.status_code == 200 and res.json() == login_pw:
                    ...
                    # 1. 안내 메시지 먼저 띄우기
                    st.success(f"✅ 로그인 성공! 보안 키를 발급 중입니다... 🚀")
                    
                    # 2. 스마트폰 브라우저에 쿠키 굽기 명령
                    if auto_login:
                    ...
                    # 3. ★ 핵심: 스마트폰이 하드디스크에 쿠키를 저장할 시간(1.5초) 벌어주기
                    time.sleep(1.5) 
...
# ★ 완벽 로그아웃 (0.001초 도망 방지 작전) ★
if st.sidebar.button("🔒 로그아웃 (다른 계정 접속)", type="secondary"):
    st.sidebar.warning("⏳ 안전하게 캐시를 삭제 중입니다...")
    
    # 1. 브라우저에 삭제 명령 하달
    cookie_manager.delete("saved_username")
    
    # 2. 확실히 지워질 때까지 1.5초 동안 파이썬 정지 (대기)
    time.sleep(1.5)
    
    # 3. 그 다음 메모리 초기화 후 화면 전환
    for key in list(st.session_state.keys()): del st.session_state[key]
    st.session_state.force_logout = True # 혹시 모를 유령 쿠키 철벽 방어막
    st.rerun()
