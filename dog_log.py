...
# 0. 기본 설정
# ==========================================
APP_VERSION = "v14.2.2 (기존 UI 보존 & 3:2 관제 텍스트 추가)"
            ...
            if login_id and login_pw:
                try:
                    ...
                    if res.status_code == 200 and res.json() == login_pw:
                        if auto_login: cookie_manager.set("saved_username", login_id, expires_at=datetime.now() + timedelta(days=180))
                        else: cookie_manager.delete("saved_username")
        ...
        if st.button("계정 생성 💾", use_container_width=True):
            if reg_id and reg_pw:
                try:
                    ...
                    if res.status_code == 200 and res.json() is not None: st.error("❌ 이미 존재하는 아이디")
                    else:
                        ...
                        requests.put(f"{FIREBASE_URL}users/{reg_id}/settings.json", json={"btn_h": 4.2, "hdr_color": "#64748b", "pee_interval": 5.0, "tg_enabled": True, "tg_token": "8560607237:AAH1HTdbxFsWGS8UFoNPAKsfmxr9wd2VNS0", "tg_chat_id": "8124116628", "order": {"타이머":1, "누적데이터":2, "배변기록":3, "산책기록":4, "건강미용":5, "수동조절":6, "기록차감":7, "활동로그":8, "주간통계":9, "가계부":10}}, timeout=5)
                        st.success("✅ 생성 완료! 로그인 탭에서 접속하세요.")
                except: st.error("⚠️ 네트워크 오류.")
    st.stop()
    ...
    def job():
        ...
        while True:
            try:
                ...
                settings = s_res.json()
                if not settings.get("tg_enabled") or not settings.get("tg_token") or not settings.get("tg_chat_id"): continue
                ...
                for k in sorted(logs.keys(), reverse=True):
                    ...
                    if "소변" in act and not any(x in act for x in ["차감", "리셋", "끄기", "알림 발송"]):
                        if '(수정)' in act and '[' in act and ']' in act:
                            ext_time = act.split('[')[1].split(']')[0]; date_part = k.split(' ')[0]; p_iso = f"{date_part}T{ext_time}+09:00"
                        else:
                            t = k.split('_')[0] if '_' in k else k.split('.')[0]; p_iso = t.replace(" ", "T") + "+09:00"
                        break
                
                if p_iso:
                    dt_iso = datetime.fromisoformat(p_iso)
                    diff_m = (datetime.now(timezone(timedelta(hours=9))) - dt_iso).total_seconds() / 60.0
                    if diff_m >= interval_m and p_iso != last_alerted:
                        ...
                        try:
                            ...
                            requests.patch(f"{FIREBASE_URL}users/{user_id}/logs.json", json={alert_ts: f"📱 알림 발송 (소변 후 {time_str} 초과)"}, timeout=5)
                        except: pass
            except: pass 
...
div[data-testid="stExpander"] {{
    ...
    border-radius: 20px !important; box-shadow: 0 8px 24px rgba(149, 157, 165, 0.08) !important;
...
div.stButton > button {{
    ...
    font-weight: 800 !important; font-size: 1.05rem !important; color: #334155 !important;
    box-shadow: 0 4px 10px rgba(0,0,0,0.03) !important; transition: all 0.2s ease !important;
}} 
...
with st.sidebar:
    ...
    with st.expander("📱 텔레그램 알림 설정", expanded=False):
        ...
        if st.button("텔레그램 설정 저장", use_container_width=True, type="primary"):
            st.session_state.settings.update({'tg_enabled': tg_enabled, 'tg_token': tg_token, 'tg_chat_id': tg_chat_id}); save_settings(st.session_state.settings); st.success("저장됨!"); st.rerun()
    ...
    with st.expander("⏰ 소변 알람 시간 설정", expanded=False):
        ...
        with col1: new_interval = st.number_input("간격", min_value=0.5, max_value=24.0, value=float(st.session_state.settings.get('pee_interval', 5.0)), step=0.5, label_visibility="collapsed")
        with col2: 
            st.markdown("<div class='save-btn-col'>", unsafe_allow_html=True)
            if st.button("💾 저장", key="btn_save_int", use_container_width=True): st.session_state.settings['pee_interval'] = new_interval; save_settings(st.session_state.settings); st.rerun()
    ...
    with st.expander("🎨 화면 테마 및 배치 순서 설정", expanded=False):
        ...
        if st.button("UI/배치 설정 저장", use_container_width=True, type="primary"):
            st.session_state.settings.update({'btn_h': new_btn_h, 'hdr_color': new_hdr_c, 'order': new_order}); save_settings(st.session_state.settings); st.rerun() 
    ...
    with st.expander("📝 반려견 정보 수정"):
        ...
        if st.button("☁️ 정보 저장", use_container_width=True):
            st.session_state.profile.update({"pet_name":p_name,"birth":p_birth, "weight":p_weight,"gender":p_gender,"memo":p_memo}); save_profile(st.session_state.profile); st.success("✅ 저장 완료!"); st.rerun()

    ldg_data = st.session_state.pet_ledger
    cur_month_str = now_kst().strftime("%Y-%m")
    monthly_total_val = int(ldg_data[ldg_data["날짜"].astype(str).str.startswith(cur_month_str)]["금액"].sum()) if not ldg_data.empty else 0
    st.markdown(f"<div style='background: linear-gradient(135deg, #ffffff, #f1f5f9); border: 1px solid #e2e8f0; border-radius: 20px; padding: 18px; margin-top: 25px; box-shadow: 0 4px 15px rgba(0,0,0,0.04);'><div style='display: flex; align-items: center; gap: 8px; margin-bottom: 8px;'><span style='font-size: 1.2rem;'>💰</span><span style='font-size: 0.95rem; font-weight: 800; color: #64748b;'>{now_kst().strftime('%m월')} 총지출</span></div><div style='font-size: 1.4rem; font-weight: 900; color: #1e293b; letter-spacing: -0.5px;'>{monthly_total_val:,}<span style='font-size: 1rem; margin-left: 2px;'>원</span></div></div>", unsafe_allow_html=True)
...
def get_latest(kw):
    ...
    m = df[df['활동'].str.contains(kw, na=False)].sort_values('시간')
    if m.empty: return None, ""
    return str(m.iloc[-1]['시간']), str(m.iloc[-1]['활동'])
...
p_time_raw, _ = get_latest("소변")
a_time_raw, _ = get_latest("알림 발송")
d_time_raw, _ = get_latest("대변")
...
if p_time_raw:
    p_dt = datetime.strptime(p_time_raw.split('_')[0], "%Y-%m-%d %H:%M:%S")
    ...
    if a_time_raw and a_time_raw > p_time_raw:
        ...
        msg_color = "#10b981" # 초록색 완료
    else:
        msg_status = "대기 중"
        msg_color = "#f59e0b" # 주황색 대기

d_disp = d_time_raw[11:16] if d_time_raw else "--:--"
d_iso = d_time_raw.split('_')[0].replace(" ","T")+"+09:00" if d_time_raw else ""

# ==========================================
# 🧱 UI 모듈 (3:2 게이지 부활 + 텍스트)
# ==========================================
def render_timer():
        ...
        function update() {{ 
            ...
            if(p_iso) {{
                ...
                if(diff >= 0) {{
                    ...
                    const r_h = Math.floor(r_abs/3600000), r_m = Math.floor((r_abs%3600000)/60000), r_s = Math.floor((r_abs%60000)/1000);
                    
                    if (rem > 0) {{
                        p_rem.innerText = String(r_h).padStart(2,'0') + ":" + String(r_m).padStart(2,'0') + ":" + String(r_s).padStart(2,'0');
            ...
            if(d_iso) {{
                ...
                if(diff >= 0) {{
                    const d_h = Math.floor(diff/3600000), d_m = Math.floor((diff%3600000)/60000), d_s = Math.floor((diff%60000)/1000);
                    d_el.innerText = String(d_h).padStart(2,'0') + ":" + String(d_m).padStart(2,'0') + ":" + String(d_s).padStart(2,'0');
...
def render_health_beauty():
    ...
    def get_d_day_info_loc(kw):
        if df.empty: return "기록 없음", ""
        matches = df[df['활동'].str.contains(kw, na=False)].sort_values('시간')
        if matches.empty: return "기록 없음", ""
        last_dt_str = str(matches.iloc[-1]['시간'])[:10]
        diff = (now_kst().date() - datetime.strptime(last_dt_str, "%Y-%m-%d").date()).days
        return last_dt_str, f"<span class='d-day-badge'>{diff}일 경과</span>" if diff > 0 else "<span class='d-day-badge'>오늘 완료</span>"
    def get_rec_for_date(kw, target_str):
        matches = df[df['시간'].astype(str).str.startswith(target_str) & df['활동'].str.contains(kw, na=False)].sort_values('시간')
        if matches.empty: return target_str, "기록 없음"
        act = str(matches.iloc[-1]['활동'])
        return target_str, act.split(":", 1)[1].strip() if ":" in act else act
        
    l_mh, d_mh = get_d_day_info_loc("🏥 병원/약")
    l_gr, d_gr = get_d_day_info_loc("✂️ 미용") 
    with st.expander("✨ 상세 기록 관리 (약/병원/미용 메모)", expanded=False):
        ts = now_kst().strftime("%H:%M:%S_%f")
        saved_d_mh = datetime.strptime(st.session_state.settings.get("sel_date_mh", now_kst().strftime("%Y-%m-%d")), "%Y-%m-%d").date()
        saved_d_gr = datetime.strptime(st.session_state.settings.get("sel_date_gr", now_kst().strftime("%Y-%m-%d")), "%Y-%m-%d").date()
        ...
        with c1:
            ...
            if d_val != saved_d_mh:
                st.session_state.settings["sel_date_mh"] = d_val.strftime("%Y-%m-%d"); save_settings(st.session_state.settings); st.rerun()
            t_val = st.text_input("메모", key="t_mh", placeholder="예: 심장사상충")
            if st.button("🏥 기록 저장", use_container_width=True): add_record(f"🏥 병원/약: {t_val}" if t_val else "🏥 병원/약", f"{d_val} {ts}")
        with c2:
            date_mh, memo_mh = get_rec_for_date("🏥 병원/약", d_val.strftime("%Y-%m-%d"))
        ...
        with c3:
            ...
            if d_grv != saved_d_gr:
                st.session_state.settings["sel_date_gr"] = d_grv.strftime("%Y-%m-%d"); save_settings(st.session_state.settings); st.rerun()
            t_grv = st.text_input("메모", key="t_gr", placeholder="예: 전체미용")
            if st.button("✂️ 기록 저장", use_container_width=True): add_record(f"✂️ 미용: {t_grv}" if t_grv else "✂️ 미용 및 목욕", f"{d_grv} {ts}")
        with c4:
            date_gr, memo_gr = get_rec_for_date("✂️ 미용", d_grv.strftime("%Y-%m-%d"))
...
def render_manual():
    with st.expander("⚙️ 타이머 수동 조절"):
        t1, t2 = st.tabs(["💧 소변", "💩 대변"])
        def add_rec_man(act, kw):
            add_record(f"{act}(수정) [{st.session_state[kw].strftime('%H:%M:%S')}] (통계제외)")
        with t1:
            st.time_input("시간 선택", now_kst().time(), key="p_wheel")
            if st.button("시간 저장", key="bp_w", use_container_width=True): add_rec_man("💦 소변", "p_wheel")
            if st.button("🔄 리셋", use_container_width=True, key="bp_r"): add_record("💦 소변 리셋 (통계제외)")
        with t2:
            st.time_input("시간 선택", now_kst().time(), key="d_wheel")
            if st.button("시간 저장", key="bd_w", use_container_width=True): add_rec_man("💩 대변", "d_wheel")
...
def render_log():
    with st.expander(f"📋 활동 로그 ({len(target_df)}건)", expanded=False):
        ...
        if not target_df.empty:
            ...
            st.dataframe(log_display.sort_values('시간', ascending=False).reset_index(drop=True), use_container_width=True)

def render_ledger():
    with st.expander("💰 반려견 가계부 (상세)", expanded=False):
        CATS = ["사료", "간식", "배변패드", "의료비", "기타"]
        ldg = st.session_state.pet_ledger
        monthly_ldg = ldg[ldg["날짜"].astype(str).str.startswith(now_kst().strftime("%Y-%m"))].copy() if not ldg.empty else pd.DataFrame()
        ...
        with lc1: l_date = st.date_input("날짜", value=now_kst().date(), key="l_date"); l_cat = st.selectbox("카테고리", CATS, key="l_cat")
        with lc2: l_amt = st.number_input("금액", min_value=0, step=1000, key="l_amt"); l_memo = st.text_input("메모", key="l_memo")
        if st.button("💾 지출 저장", use_container_width=True, type="primary") and l_amt > 0:
        ...
        if not monthly_ldg.empty:
            ...
            if st.button(f"❌ 직전 삭제: {last_row['카테고리']} {int(last_row['금액']):,}원", use_container_width=True): delete_ledger_entry(str(last_row["키"]))
...
st.markdown(f"<div style='text-align:center; color:#94a3b8; font-size:0.75rem; padding:20px 0 30px; font-weight:600;'>🐾 Smart Pet Care Center<br>현재 버전: <strong>{APP_VERSION}</strong> | 업데이트: {UPDATE_DATE}</div>", unsafe_allow_html=True)
