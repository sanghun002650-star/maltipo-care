...
# 0. 기본 설정
# ==========================================
APP_VERSION = "v14.1.10 (가로 배치 강제 고정 & 잔상 원천 차단)"
                ...
                for k in sorted(logs.keys(), reverse=True):
                    ...
                    if "소변" in act and not any(x in act for x in ["차감", "리셋", "끄기", "알림 발송"]):
                        if '(수정)' in act and '[' in act and ']' in act:
                            ...
                            date_part = k.split(' ')[0]
                            p_iso = f"{date_part}T{ext_time}+09:00"
                        else:
                            t = k.split('_')[0] if '_' in k else k.split('.')[0]
                            p_iso = t.replace(" ", "T") + "+09:00"
                        break
...
/* 🚨 [중요] 차트 툴팁 잔상 방지 패치 (원천 차단) 🚨 */
#vg-tooltip-element, .vg-tooltip, .vega-bindings {{ 
    display: none !important; 
    visibility: hidden !important; 
    opacity: 0 !important; 
    pointer-events: none !important; 
}}
/* 차트 상호작용 원천 차단으로 포커스 오류 방지 */
...
# ==========================================
# ⚙️ 사이드바 (v14.1.8의 간결한 2열 배치 유지)
# ==========================================
with st.sidebar:
    st.markdown(f"### ⚙️ {username}님")
    if st.button("🔒 로그아웃", use_container_width=True):
        cookie_manager.delete("saved_username")
        st.session_state.force_logout = True; st.session_state.logged_in = False
        time.sleep(0.3); st.rerun()
    ...
    with st.expander("⏰ 소변 알람 시간 설정", expanded=False):
        ...
        with col1:
            new_interval = st.number_input("간격", min_value=0.5, max_value=24.0, value=float(st.session_state.settings.get('pee_interval', 5.0)), step=0.5, label_visibility="collapsed")
        with col2:
...
def render_timer():
    ...
    # [수정] flex-direction: row 강제 적용으로 세로 무너짐 방지
    components.html(f"""
    <div style="display: flex; flex-direction: row; justify-content: space-between; gap: 15px; font-family: sans-serif; width: 100%;">
...
def render_summary():
    ...
    # [수정] 세로로 무너지는 현상 차단을 위해 display: flex; flex-direction: row 강제 하드코딩
    st.markdown(f"""
