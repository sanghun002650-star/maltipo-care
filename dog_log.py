...
def render_timer():
    ...
    def _get_time(k):
        ...
        last_act = str(m.iloc[-1]['활동'])
        last_time = str(m.iloc[-1]['시간'])
        
        # 수동 조절된 기록인 경우 활동 텍스트 내부에서 시간(HH:MM:SS) 추출
        if '(수정)' in last_act and '[' in last_act and ']' in last_act:
            t_val = last_act.split('[')[1].split(']')[0]
            d_val = last_time[:10]
            return f"{d_val}T{t_val}+09:00"
        
        # 일반 기록인 경우 정상적으로 키(시간) 사용 - Syntax Error 완벽 해결
    ...
    <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px; font-family:sans-serif; margin-bottom:4px;">
        <div style="background:linear-gradient(160deg,#dcfce7,#bbf7d0); border-radius:20px; padding:16px 10px; text-align:center; box-shadow:0 4px 15px rgba(34,197,94,0.25); border: 2px solid #86efac;">
            <div style="font-size:0.8rem; font-weight:900; color:#15803d;">💧 소변 경과</div>
            <div id="p_tm" style="font-size:2.6rem; font-weight:900; color:#14532d; letter-spacing:2px; line-height:1.1; margin:6px 0;">--:--</div>
            <div style="background:rgba(255,255,255,0.6); border-radius:8px; padding:3px 6px; font-size:0.72rem; font-weight:800; color:#166534;">마지막 {p_str}</div>
        </div>
        <div style="background:linear-gradient(160deg,#fff7ed,#fed7aa); border-radius:20px; padding:16px 10px; text-align:center; box-shadow:0 4px 15px rgba(249,115,22,0.25); border: 2px solid #fdba74;">
            <div style="font-size:0.8rem; font-weight:900; color:#c2410c;">💩 대변 경과</div>
            <div id="d_tm" style="font-size:2.6rem; font-weight:900; color:#7c2d12; letter-spacing:2px; line-height:1.1; margin:6px 0;">--:--</div>
            <div style="background:rgba(255,255,255,0.6); border-radius:8px; padding:3px 6px; font-size:0.72rem; font-weight:800; color:#9a3412;">마지막 {d_str}</div>
        </div>
    </div>
    <script>
        function up(id, iso){{
            const el=document.getElementById(id); if(!iso){{el.innerText="--:--";return;}}
        ...
        setInterval(()=>{{up('p_tm',"{p_iso}");up('d_tm',"{d_iso}");}},1000); up('p_tm',"{p_iso}");up('d_tm',"{d_iso}");
    </script>
    """, height=145)
...
def render_deduct():
    with st.expander("➖ 잘못 누른 기록 차감"):
        ...
        with a1:
            if st.button("💧 소변-1"): add_record("💦 소변 차감 (-1)")
        with a2:
            if st.button("💩 대변-1"): add_record("💩 대변 차감 (-1)")
        with a3:
            if st.button("🦮 산책-1"): add_record("🦮 산책 차감 (-1)")
...
# 업데이트 정보 표기 (요청 사항 반영)
# 업데이트 버전 정보: v13.5.0 (Manual Sync Patch Syntax Fix)
# 업데이트 날짜: 2026-04-01

# ENDimport streamlit as st
import pandas as pd
from datetime import datetime, timezone, timedelta
import streamlit.components.v1 as components
import requests
