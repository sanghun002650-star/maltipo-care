...
# 0. 기본 설정
# ==========================================
APP_VERSION = "v14.2.1 (최고 가독성 & 확정 상태 관제)"
        ...
        while True:
            try:
                ...
                for k in sorted(logs.keys(), reverse=True):
                    if "소변" in str(logs[k]) and not any(x in str(logs[k]) for x in ["차감","리셋","알림"]):
                        p_ts = k.split('_')[0]
                        break
...
def get_latest(kw):
    ...
    match = df[df['활동'].str.contains(kw, na=False)].sort_values("시간")
    if match.empty: return None, ""
    last = match.iloc[-1]
    return last['시간'], last['활동']
...
def render_timer():
    ...
    <div style="display:flex; flex-direction:row; gap:15px; font-family:'Malgun Gothic', sans-serif; width: 100%;">
        ...
        <div style="flex:3; background:#ffffff; border-radius:20px; padding:20px; box-shadow:0 4px 15px rgba(0,0,0,0.04); border: 2px solid #e0f2fe; display: flex; flex-direction: column; justify-content: space-between;">
            <div style="font-size:1.1rem; font-weight:900; color:#0284c7; margin-bottom:15px; text-align:center;">💧 소변 관제</div>
            
            <div style="display:flex; justify-content: space-between; align-items: center;">
                <div style="text-align:center; flex:1;">
                    <div style="font-size:0.8rem; font-weight:700; color:#64748b; margin-bottom:2px;">남은 시간</div>
                    <div id="p_rem" style="font-size:2.2rem; font-weight:900; color:#0369a1; letter-spacing:1px; line-height:1.2;">--:--:--</div>
                </div>
                
                <div style="border-left: 2px dashed #f1f5f9; padding-left: 15px; flex:1; display:flex; flex-direction:column; gap:6px;">
                    ...
                    <div style="margin-top: 5px; font-size: 0.8rem; font-weight: 800; color: {msg_color}; background: #f8fafc; padding: 4px 8px; border-radius: 6px; text-align:center;">[{msg_status}]</div>
                </div>
            </div>
        </div>

        <div style="flex:2; background:#ffffff; border-radius:20px; padding:20px; box-shadow:0 4px 15px rgba(0,0,0,0.04); border: 2px solid #ffedd5; text-align:center; display: flex; flex-direction: column; justify-content: center;">
            <div style="font-size:1.1rem; font-weight:900; color:#c2410c; margin-bottom:10px;">💩 대변</div>
            <div style="font-size:0.8rem; font-weight:700; color:#64748b; margin-bottom:2px;">경과 시간</div>
            <div id="d_elap" style="font-size:2.2rem; font-weight:900; color:#9a3412; letter-spacing:1px; line-height:1.2;">--:--:--</div>
            <div style="font-size:0.9rem; color:#334155; margin-top:12px; border-top: 2px dashed #f1f5f9; padding-top: 8px;">최근 <span style="font-weight:900;">{d_disp}</span></div>
        </div>
        
    </div>
    
    <script>
        const P_LIM = {intv_h} * 3600000;
        function upd() {{
            const now = new Date();
            const p_iso = "{p_iso}";
            if(p_iso) {{
                ...
                if(diff >= 0) {{
                    ...
                    const sign = rem < 0 ? "-" : "";
                    const rem_el = document.getElementById('p_rem');
                    rem_el.innerText = sign + String(r_h).padStart(2,'0') + ":" + String(r_m).padStart(2,'0') + ":" + String(r_s).padStart(2,'0');
                    if (rem < 0) {{ rem_el.style.color = "#ef4444"; }} else {{ rem_el.style.color = "#0369a1"; }}
                }}
            }}
            const d_iso = "{d_iso}";
            if(d_iso) {{
                ...
                if(diff >= 0) {{
                    ...
                    document.getElementById('d_elap').innerText = String(d_h).padStart(2,'0') + ":" + String(d_m).padStart(2,'0') + ":" + String(d_s).padStart(2,'0');
                }}
            }}
        }}
...
if not df.empty:
    last = df.iloc[-1]
    if st.button(f"❌ 취소: {last['활동']}", use_container_width=True):
